<?php

include("lib/password.php");
require("authentication.php");


if (isset($_REQUEST['username']) && isset($_REQUEST['password'])) {
        global $dbConn;
	$username = mysql_real_escape_string($_REQUEST['username']);
        $pass = mysql_real_escape_string($_REQUEST['password']);
	// Built-in hashing func
	$hash = $dbConn->getSingle("SELECT password FROM passwords WHERE username='$username'");
	if (password_verify($pass, $hash)) {
		$auth  = new Auth();
		$token = $auth->issueToken($username);
		print <<<EOF
			<script>
			localStorage.setItem("forumToken", "$token");
			localStorage.setItem("forumUsername", "$username");
			location.href = "/feed"; 
			</script>
EOF;
	} else {
		print("Incorrect user/pass");
	}	
}



print <<<EOF
<section class="vh-100">
  <div class="container-fluid">
    <div class="row">
      <div class="col-sm-6 text-black">

        <div class="d-flex align-items-center h-custom-2 px-5 ms-xl-4 mt-5 pt-5 pt-xl-0 mt-xl-n5">

          <form action=signin method='POST' style="width: 23rem;">

            <h3 class="fw-normal mb-3 pb-3" style="letter-spacing: 1px;">Log in</h3>

            <div class="form-outline mb-4">
              <input name="username" id="form2Example18" class="form-control form-control-lg" />
              <label class="form-label" for="form2Example18">Username</label>
            </div>

            <div class="form-outline mb-4">
              <input name="password" type="password" id="form2Example28" class="form-control form-control-lg" />
              <label class="form-label" for="form2Example28">Password</label>
            </div>
		

            <div class="pt-1 mb-4">
              <button class="btn btn-info btn-lg btn-block" type="submit">Login</button>
            </div>

            <p class="small mb-5 pb-lg-2"><a class="text-muted" href="#!">Forgot password?</a></p>
            <p>Don't have an account? <a href="/createAccount" class="link-info">Register here</a></p>

          </form>

        </div>

      </div>
      <div class="col-sm-6 px-0 d-none d-sm-block">

        <img src="https://ichef.bbci.co.uk/news/976/cpsprodpb/292A/production/_121183501_gettyimages-1053558054.jpg" alt="Login image" class="w-100 vh-100" style="object-fit: cover; object-position: center;">
      </div>
    </div>
  </div>
</section>	
EOF;

?>

